...
import { getCompetitionData, CompetitionId } from '../../../utils/leagues'
...
export const standingsRouter = router({
  seasonStandings: publicProcedure
    ...
    .query(async ({ input }) => {
      const { seasonId, stageId } = getCompetitionData(input.competitionId)

      const path = [
        'calendar',
        input.competitionId,
        seasonId,
        input.stageId || isString(stageId)
          ? stageId
          : Object.values(stageId)[0],
        'Standing',
      ].join('/')
  ...
  groupedStandings: publicProcedure
    ...
    .query(async ({ input }) => {
      const league = getCompetitionData(input.competitionId)
